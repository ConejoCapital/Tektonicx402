...
### 4. Seller Revenue Extraction (Solana)
...
### 5. Network Failure Rate Diagnostics (Solana)
...
### 6. EVM Facilitator Application Dominance (Base)
...
### 7. Base Network Daily User Retention (Base)
...
### 8. Cross-Chain Universal x402 Economy (Solana + Base)
...
SELECT 
  ...
  ROUND(SUM(CASE WHEN chain = 'solana' THEN amount_usdc ELSE 0 END), 2) AS solana_volume_usdc,
  ROUND(SUM(CASE WHEN chain = 'base' THEN amount_usdc ELSE 0 END), 2) AS base_volume_usdc,
  ROUND(SUM(amount_usdc), 2) AS combined_global_volume_usdc
FROM global_x402
