...
# BigQuery Table creation query for the temporary US Dataset
create_table_sql = """
CREATE TABLE IF NOT EXISTS `web3-publicgoods.tektonic_temp_base_us.base_raw` (
    tx_hash STRING,
...
OPTIONS(
  description="Historical x402 extraction for Base Network (US Location)"
);
"""
...
CREATE TEMP TABLE x402_facilitators (address STRING, facilitator STRING);
...
INSERT INTO `web3-publicgoods.tektonic_temp_base_us.base_raw`
SELECT
  ...
  -- Account for successful vs failed dynamically 
  CASE WHEN tx.status = 1 THEN SAFE_DIVIDE(CAST(t.quantity AS FLOAT64), 1e6) ELSE 0 END AS amount_usdc,
  
  tx.from_address                            AS facilitator_address,
  f.facilitator                              AS facilitator_name,
  'USDC'                                     AS token_symbol,
  t.event_index                              AS log_index,
  'base'                                     AS chain
FROM `web3-publicgoods.base_w3_publicgoods.token_transfers` t
JOIN `web3-publicgoods.base_w3_publicgoods.receipts` tx
  ON t.transaction_hash = tx.transaction_hash
  AND tx.block_timestamp >= TIMESTAMP_SUB(start_ts, INTERVAL 30 MINUTE)
  AND tx.block_timestamp < TIMESTAMP_ADD(end_ts, INTERVAL 30 MINUTE)
JOIN x402_facilitators f 
  ON LOWER(tx.from_address) = LOWER(f.address)
WHERE
...
def run_bq(query):
    # Enforces routing billing entirely to web3-publicgoods under Devan's account
    # Location set to US strictly for the Base Marketplaces API dump compatibility
    res = subprocess.run(
        ["bq", "query", "--use_legacy_sql=false", "--project_id=web3-publicgoods", "--location=US", query],
        text=True
    )
    if res.returncode != 0:
...
print("Creating native partitioned base_raw table in the temp US dataset...")
...
end_date = datetime.datetime(2026, 4, 3, tzinfo=datetime.timezone.utc)
...
while current_start < end_date:
    ...
    print(f"Hydrating Base block temporal range: {start_str} to {end_str}")
    chunk_query = insert_chunk_sql_template.format(start_time=start_str, end_time=end_str)
    
    if run_bq(chunk_query):
        print(f"✅ Chunk {start_str} successfully ingested into web3-publicgoods.tektonic_temp_base_us.base_raw")
    else:
...
print("🎉 Historical Base Hydration Complete!")
